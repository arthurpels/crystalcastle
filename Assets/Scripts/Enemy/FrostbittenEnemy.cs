...
public class FrostbittenEnemy : MonoBehaviour {
  public enum EnemyState { Frozen, Thawing, Active, Dead }

  public EnemyState CurrentState { get; private set; } = EnemyState.Frozen;

  [SerializeField]
  private GameManager.SectorID boundSector;
  [SerializeField]
  ...
  private void Update() {
    ...
    if (agent.remainingDistance < 0.5f && !agent.pathPending) {
      currentWaypointIndex =
          (currentWaypointIndex + 1) % patrolWaypoints.Length;
      PatrolToNextWaypoint();
    }
  }
  ...
  private Color GetStateColor() {
    return CurrentState switch { EnemyState.Frozen => frozenColor,
